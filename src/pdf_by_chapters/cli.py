...
@app.command(rich_help_panel="Review")
def review(
    directory: Path = typer.Argument(
        ..., help="Directory containing downloads (flashcards/, quizzes/)."
    ),
    flashcards_only: bool = typer.Option(
        False, "--flashcards", "-f", help="Only review flashcards."
    ),
    quiz_only: bool = typer.Option(False, "--quiz", "-q", help="Only review quizzes."),
    shuffle: bool = typer.Option(True, "--shuffle/--no-shuffle", help="Shuffle cards/questions."),
    limit: int | None = typer.Option(None, "--limit", "-l", help="Max items to review."),
) -> None:
    """Interactive flashcard and quiz review in the terminal.

    Point at a downloads directory (or its parent) containing flashcards/
    and quizzes/ subdirectories generated by from-obsidian.

    \b
    Examples:
      pdf-by-chapters review ~/Desktop/ZTM-DE/downloads
      pdf-by-chapters review ~/Desktop/ZTM-DE/downloads --flashcards
      pdf-by-chapters review ~/Desktop/ZTM-DE/downloads --quiz --no-shuffle
    """
    ...
    fc_dir, quiz_dir = discover_content(resolved)

    if not fc_dir and not quiz_dir:
        console.print("[red]No flashcard or quiz files found.[/red]")
        console.print("[dim]Expected *-flashcards.json and/or *-quiz.json files.[/dim]")
        raise typer.Exit(1)

    # Determine what to review
    do_flashcards = not quiz_only and fc_dir is not None
    do_quizzes = not flashcards_only and quiz_dir is not None

    if do_flashcards and fc_dir:
        cards = load_flashcards(fc_dir)
        if cards:
            cards = shuffle_items(cards, shuffle)
            if limit:
                cards = cards[:limit]
            console.print(
                Panel(
                    f"[bold]Flashcard Review[/bold] — {len(cards)} cards\n"
                    "[dim]Press Enter to reveal, then score yourself (y/n/s to skip)[/dim]",
                    border_style="cyan",
                )
            )
            result = _run_flashcard_review(cards)
            _show_result_summary("Flashcards", result)

    if do_quizzes and quiz_dir:
        questions = load_quizzes(quiz_dir)
        if questions:
            questions = shuffle_items(questions, shuffle)
            if limit:
                questions = questions[:limit]
            console.print(
                Panel(
                    f"[bold]Quiz Review[/bold] — {len(questions)} questions\n"
                    "[dim]Type the letter of your answer (a/b/c/d), h for hint, s to skip[/dim]",
                    border_style="green",
                )
            )
            result = _run_quiz_review(questions)
            _show_result_summary("Quiz", result)


def _run_flashcard_review(cards: list) -> Any:
    """Run interactive flashcard session. Returns ReviewResult."""
    from rich.panel import Panel

    from pdf_by_chapters.review import ReviewResult

    result = ReviewResult(total=len(cards))

    for i, card in enumerate(cards, 1):
        hdr = f"\n[dim]Card {i}/{len(cards)}[/dim]  [dim italic]{card.source}[/dim italic]"
        console.print(hdr)
        console.print(Panel(card.front, title="Question", border_style="cyan"))

        try:
            input("  Press Enter to reveal answer...")
        except (KeyboardInterrupt, EOFError):
            console.print("\n[yellow]Review ended early.[/yellow]")
            result.skipped = len(cards) - i + 1
            break

        console.print(Panel(card.back, title="Answer", border_style="green"))

        try:
            reply = input("  Did you know it? (y/n/s=skip) ").strip().lower()
        except (KeyboardInterrupt, EOFError):
            console.print("\n[yellow]Review ended early.[/yellow]")
            result.skipped = len(cards) - i
            break

        if reply in ("y", "yes"):
            result.correct += 1
            console.print("  [green]Correct[/green]")
        elif reply in ("s", "skip"):
            result.skipped += 1
...
def _run_quiz_review(questions: list) -> Any:
    """Run interactive quiz session. Returns ReviewResult."""
    from rich.panel import Panel

    from pdf_by_chapters.review import ReviewResult

    result = ReviewResult(total=len(questions))
    letters = "abcdefghij"

    for i, q in enumerate(questions, 1):
        hdr = f"\n[dim]Question {i}/{len(questions)}[/dim]  [dim italic]{q.source}[/dim italic]"
        console.print(hdr)
        console.print(Panel(q.question, title="Question", border_style="green"))

        # Show options
        for j, opt in enumerate(q.options):
            console.print(f"  [bold]{letters[j]})[/bold] {opt.text}")

        # Get answer
        answered = False
        while not answered:
            try:
                reply = input("\n  Your answer (a/b/c/d, h=hint, s=skip): ").strip().lower()
            except (KeyboardInterrupt, EOFError):
                console.print("\n[yellow]Review ended early.[/yellow]")
                result.skipped = len(questions) - i + 1
                return result
            ...
            if reply == "s":
                ...
                console.print("  [dim]Skipped[/dim]")
                answered = True
            elif reply == "h" and q.hint:
                console.print(f"  [yellow]Hint:[/yellow] {q.hint}")
            elif reply in letters[: len(q.options)]:
                idx = letters.index(reply)
                ...
                if chosen.is_correct:
                    ...
                    result.incorrect += 1
                    correct_idx = next((j for j, o in enumerate(q.options) if o.is_correct), 0)
                    console.print(
                        f"  [red]Incorrect.[/red] The answer was"
                        f" [bold]{letters[correct_idx]})[/bold]"
                    )

                if chosen.rationale:
                    console.print(f"  [dim]{chosen.rationale}[/dim]")

                # Show correct rationale if they got it wrong
                if not chosen.is_correct:
                    correct_opt = next((o for o in q.options if o.is_correct), None)
                    if correct_opt and correct_opt.rationale:
                        console.print(f"  [green dim]{correct_opt.rationale}[/green dim]")
            else:
                last = letters[len(q.options) - 1]
                console.print(f"  [dim]Enter a-{last}, h for hint, or s to skip[/dim]")

    return result


def _show_result_summary(label: str, result: Any) -> None:
    """Display review session summary."""
    console.print(f"\n{'─' * 50}")
    console.print(f"[bold]{label} Summary[/bold]")

    total_attempted = result.correct + result.incorrect
    if total_attempted > 0:
        pct = result.correct / total_attempted * 100
        if pct >= 80:
        ...
        console.print(
            f"  Score: [bold]{result.correct}/{total_attempted}[/bold] ({pct:.0f}%) — {grade}"
        )
    else:
        console.print("  No questions attempted")
